...
# 导入RetinaFace检测器
sys.path.append('/root/Robot_pet/face_ws/face_detection')
from face_detector_core import FaceDetector

# 加载ONNX模型
model_path = '/root/Robot_pet/face_ws/face_recognition/mobile_facenet.onnx'
...
# 初始化RetinaFace人脸检测器
retinaface_model_path = "/root/Robot_pet/face_ws/face_detection/retinaface.onnx"
face_detector = FaceDetector(retinaface_model_path, confidence=0.6)

# 在程序开始时初始化数据库
face_sqlite.init_db()

def detect_face(frame, save_dir="face_images"):
    """
    使用RetinaFace检测人脸，返回与原函数兼容的格式
    """
    # 使用RetinaFace检测
    ...
    return len(face_images), face_images, face_locations
...
def face_recognization():
    ...
    try:
        ...
        while not exit_flag.is_set():
            ...
            # 调用人脸检测函数
            face_count, face_images, face_locations = detect_face(frame)
